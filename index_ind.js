...
// Add this method to your PublicVideoPortal class
async filterByCategory(categorySlug, parentSlug) {
  this.isLoading = true;
  this.showSkeletonLoader();
  ...
  try {
    ...
    if (parentSlug) {
      // If we have a parent slug, this is a subcategory
      query = query.eq('subcategory', categorySlug);
    } else {
    ...
    this.showError('Failed to filter videos');
  } finally {
    this.isLoading = false;
...
class CategoryDisplay {
  ...
  // Method to render the category menu
  renderCategoryMenu(container) {
    container.innerHTML = this.categories.map(category => `
      <div class="group">                   
        <a href="#" class="flex items-center px-4 py-3 rounded-md hover:bg-opacity-20 hover:bg-white" 
        ...
        <div class="drop-up-menu absolute hidden group-hover:block bg-white shadow-md rounded-md mt-2 py-2 z-10">
          ${category.subcategories.map(sub => `
            <a href="#" class="drop-up-item category-link" 
  ...
  initializeListeners(container) {
    ...
    container.querySelectorAll('[data-category]').forEach(link => {
      link.addEventListener('click', (e) => {
        ...
        const parentSlug = e.target.dataset.parent;
        
        // Emit custom event for category selection
...
document.addEventListener('DOMContentLoaded', async () => {
  const categoryDisplay = new CategoryDisplay(supabase);
  
  try {
    await categoryDisplay.loadCategories();
    const menuContainer = document.querySelector('.category-menu');
    categoryDisplay.renderCategoryMenu(menuContainer);
    categoryDisplay.initializeListeners(menuContainer);

    // Listen for category selection
    document.addEventListener('categorySelected', (e) => {
      const { category, parent } = e.detail;
      // Filter videos based on category/subcategory
      videoPortal.filterByCategory(category, parent);
    });
